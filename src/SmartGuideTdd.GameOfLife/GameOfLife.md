...
...implement your own method which will take the initial state as an NxM grid of living vs dead cells 
and return an equally sized grid representing the next generation. Cells 
outside the grid must be considered dead. Cells that would born out of the grid boundaries should 
be ignored (universe never grows beyond the initial NxM grid).